...
def recognize(wav_filename, args):
    ...
    with open(video_name+'_'+folder+'_'+args.language+ '.txt', 'w', encoding='utf-8') as f:
...
def punctuate_text(text, args):
    ...
    with open( filepath+'_'+folder+'_'+args.algorithm_summary+'_processed_text.txt', 'w', encoding='utf-8') as file:
        for sentence in sentences:
            file.write(sentence + '\n')
    return filepath+'_'+folder+'_'+args.algorithm_summary+'_processed_text.txt'
...
def save_result_to_file(result, args):
    file_path = os.path.splitext(args.video)[0] #data/video_folder
    ...
    with open(file_path+'_'+folder+'_'+args.language+'_'+args.algorithm_summary+'_summary.txt', 'w', encoding='utf-8') as file:
        file.write(result_str_translated)
    print(f"File summary at {file_path+'_'+folder+'_'+args.language+'_'+args.algorithm_summary+'_summary.txt',} successfully saved")
...
if __name__ == '__main__':
    ...
    video_name = os.path.splitext(args.video)[0]
    open(video_name+'_'+folder+'_'+args.language+'.txt', 'w', encoding = 'utf-8').write('')
    ...
    video_name = os.path.splitext(args.video)[0]
    file_path = video_name+'_'+folder +'_'+args.language+'.txt'
    ...
    if args.algorithm_summary == 'no':
        ...
        path_summary_text = 'None, None, None'
    else:    
        path_processed_text = video_name+'_'+folder+'_'+args.algorithm_summary+'_processed_text.txt'
        ...
        path_summary_text = video_name+'_'+folder+'_'+args.language+'_'+args.algorithm_summary+'_summary.txt'
        count_sentence_summary_text = count_sentence(path_summary_text)
        path_summary_text = path_summary_text+', '+count_sentence_summary_text
    if os.path.exists(history_file):
        with open(history_file, 'a') as file:
            count_sentence = count_sentence(file_path)
            read_video_info = read_video_info(args.video)
            content = folder+', '+args.video+', '+read_video_info+', '+args.language+', '+file_path+', '+count_sentence+', '+path_processed_text+', '+path_summary_text+', '+args.algorithm_noise+', '+args.algorithm_summary+', '+args.extra_argument +', '+format(end - start)
            print(content)
